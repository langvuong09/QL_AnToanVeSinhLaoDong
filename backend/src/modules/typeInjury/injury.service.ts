...
export class InjuryTypeService {
    ...
    if (dto.parentId) {
      ...
      if (!parentExist) {
        throw new NotFoundException(
          'Không tìm thấy loại chấn thương cha đã chọn hoặc danh mục đã bị xóa!',
        );
      }

      if (!parentExist.isActive) {
        throw new BadRequestException(
          'Không thể chọn loại chấn thương cha đang bị ngưng hoạt động!',
        );
      }
    }
  ...
  async update(id: number, dto: UpdateInjuryTypeDto) {
    ...
    if (dto.code && dto.code.trim() !== injury.code) {
      ...
      if (isCodeExist)
        throw new BadRequestException(
          'Mã loại chấn thương mới đã được sử dụng!',
        );
    ...
    if (dto.parentId) {
      ...
      const checkIsChildRecursive = (
        currentId: number,
        targetParentId: number,
      ): boolean => {
        const directChildren = allInjuries.filter(
      ...
      if (checkIsChildRecursive(id, dto.parentId)) {
        throw new BadRequestException(
          'Không thể chọn một loại chấn thương con hoặc cháu làm danh mục cha của nó!',
        );
      }
    }
  ...
  private async getInjuryTypesBase(query: any, onlyActive: boolean) {
    ...
    const isActive =
      query.isActive !== undefined ? String(query.isActive) === 'true' : null;

    const queryBuilder = this.injuryRepository
    ...
    if (onlyActive) {
      ...
      queryBuilder.andWhere('it.isActive = :isActive', { isActive });
    }

    if (code) {
      queryBuilder.andWhere('it.code ILike :code', {
        code: `%${code.trim()}%`,
      });
    }

    if (name) {
      queryBuilder.andWhere('it.name ILike :name', {
        name: `%${name.trim()}%`,
      });
    }
    ...
    if (isFiltering) {
      ...
      const formattedItems = allRawItems.map((item) =>
        this.mapLevelAndFormat(item),
      );
      ...
      const paginatedTree = treeData.slice(
        (page - 1) * pageSize,
        page * pageSize,
      );
  ...
  async getDetail(id: number) {
    const injury = await this.injuryRepository
      .createQueryBuilder('it')
      .leftJoinAndSelect('it.parent', 'parent')
      .where('it.id = :id', { id })
    ...
    if (!injury)
      throw new NotFoundException(
        'Không tìm thấy loại chấn thương này hoặc đã bị xóa',
      );
  ...
  async bulkRemove(ids: number[]) {
    ...
    const collectChildrenIdsRecursive = (parentIds: number[]) => {
      const children = allInjuries.filter(
        (item) => item.parentId && parentIds.includes(item.parentId),
      );
