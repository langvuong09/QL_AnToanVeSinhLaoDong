...
export class ReportController {
  ...
  @ApiOperation({
    summary:
      '🎯 Doanh nghiệp khởi tạo bản nháp báo cáo định kỳ (Kèm đính kèm tài liệu)',
  })
  ...
  @ApiOperation({
    summary:
      '🎯 Doanh nghiệp cập nhật sửa đổi nội dung bản nháp/bản bị từ chối (Chỉ sửa được khi trạng thái là DRAFT hoặc REJECTED)',
  })
  async updateReport(
  ...
  @ApiOperation({
    summary: '🎯 Cấp Quản lý (Admin) tra cứu danh sách báo cáo toàn thành phố',
  })
  ...
  @ApiQuery({
    ...
    required: false,
    type: String,
    description: 'Lọc sâu theo Tỉnh/Thành phố',
  })
  @ApiQuery({
    name: 'district',
    required: false,
    type: String,
    description: 'Lọc sâu theo Quận/Huyện',
  })
  @ApiQuery({
    name: 'ward',
    required: false,
    type: String,
    description: 'Lọc sâu theo Phường/Xã',
  })
  ...
  @ApiOperation({
    summary:
      '🎯 Doanh nghiệp tự tra cứu lịch sử nộp báo cáo của chính mình (Tự động lọc theo user.doetId)',
  })
  @ApiQuery({ name: 'page', required: false, type: Number })
  @ApiQuery({ name: 'pageSize', required: false, type: Number })
  @ApiQuery({ name: 'year', required: false, type: Number })
  ...
  @ApiOperation({
    summary:
      '🎯 Lấy dữ liệu chi tiết cấu trúc phẳng dẹt đổ thẳng vào dataSource của UI Table',
  })
  ...
  @ApiOperation({
    summary:
      '🎯 Báo cáo tổng hợp số liệu Tai nạn lao động theo từng loại hình sở hữu doanh nghiệp (Dành cho Lãnh đạo)',
  })
  ...
  @ApiOperation({
    summary:
      '🎯 Điều chuyển trạng thái báo cáo tuần tự nghiêm ngặt (DRAFT -> SUBMITTED -> APPROVED / REJECTED)',
  })
  async changeStatus(
    @Param('id', ParseIntPipe) id: number,
