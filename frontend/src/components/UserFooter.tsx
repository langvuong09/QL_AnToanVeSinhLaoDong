...
export default function UserFooter() {
  ...
  return (
    <div className="relative">
      ...
      <div className="flex items-center gap-3 py-4 px-5 cursor-pointer hover:bg-[#2447a5] border-t  border-b border-[#FFFFFF]" onClick={() => setIsClicked(prev => !prev)}>
        <div className="w-10 h-10 rounded-full bg-white overflow-hidden">
          {authenticate?.state?.avatar ? (
            <img src={authenticate?.state?.avatar.url} alt={authenticate?.state?.fullName} className="w-full h-full object-cover" />
          ) : (
            <div className="w-full h-full bg-linear-to-br from-blue-400 to-blue-600 flex items-center justify-center text-white font-bold">
              {authenticate?.state?.fullName?.charAt(0).toUpperCase()}
            </div>
          )}
        </div>
        ...
        <div className="flex-1 flex justify-between items-center">
          <div className="min-w-0">
            <h1 className="text-sm font-medium truncate">{authenticate?.state?.fullName || 'Tài khoản'}</h1>
          </div>
